...
#include "common.h"
#include "vendor.h"
void Survey_Ready(void);		//PID准备
void Survey_Controller(void);	//PID控制
void Value_Dispose(void);		//数据解析
void PIT0_init(void);			//中断定时器
void PIT1_init(void);			//中断定时器
void PIT_Task0(void);			//定时器1中断
void PIT_Task1(void);
void ASIC_DACAndInspectHandler(void);//安森美DAC命令和子检测控制句柄
void ASIC_ReadMeasurementResultHandler(void);//安森美读取测量结果控制句柄
...
uint8_t ASIC_Flag = 1;
uint8_t HEXorDEC = 0;			//0：10进制；1：16进制
...
float Kp[7][5] =	   {
							...
							{0.005,0.005,0.005,0.005,0.005},		//4: I1+查表--->V0
...
float Ki[7][5] = 	   {
							...
							{0.0002,0.0002,0.0002,0.0002,0.0002},		//4: I1+查表--->V0
...
float Kd[7][5] = 	   {
							...
							{0.0001,0.0001,0.0001,0.0001,0.0001},	//4: I1+查表--->V0
							{0,0,0,0,0},							//5: I0--->V0(2)
							{0,0,0,0,0}								//2: I2--->V2(2)
																	};
/*
...
float Vref0_V = 0.35;					//Vref0目标值
float Vref1_V = 0.405;					//Vref1目标值
int16_t adc_value[6];
...
void ASIC_DACAndInspectHandler(void){					//FTM2溢出中断		发送IP0 IP1 IP2数值控制
...
void ASIC_ReadMeasurementResultHandler(void){					//发送安森美数据采集命令
...
void MCLK_init(void)
{
	...
	FTM0CH1_Config.u16CnV=12;//占空比0.5
...
void Value_Dispose(void)		//数据解析，换算AD读取值
{
...
//===================================================================================================================
		...
		fuzzy_Controller(&IP1);			//模糊自适应，按需自动调节IP1.Ki的值

	//	if(myabs(IP1.E) > 0.1){
		float Vref0_inc;
		Vref0_inc = (IP1.Kp+IP1.Ki+IP1.Kd)*IP1.E - (IP1.Kp+2*IP1.Kd)*IP1.E_1 + IP1.Kd*IP1.E_2;
		if(myabs(Vref0.E) > 0.1){			//Vref0未到达目标值时  放缓对Vref0目标值的变化
			Vref0_inc /= 2.0;
		}else if(myabs(IP1.dE) < 1){		//Ip1抖动较小时，及Ip1初步稳定，则增大PID增量，加快Ip1闭环到7uA的速度
...
//===================================================================================================================
//	if(Vref0Value > 0.35){				//判断Vref0大于350mV，若长时间大于则拉回至350mV
//		count1++;
//		if(count1>100 * 30){
//			Vref0_V = 0.35;
//			count1 = 0;
