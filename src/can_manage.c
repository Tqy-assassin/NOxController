...
//	Param_ID
	...
	PID_IP2_I,
	PID_IP2_D,
//A1,B1,A2,B2,A3,B3,A4,B4计算氮氧值三点方法
  	Coeffi_NOx_A3,
  	Coeffi_NOx_B3,
  	Coeffi_NOx_A4,
  	Coeffi_NOx_B4,
...
extern float Vref;
uint32_t NOValueOffsetTimer = 0;	//氮氧输出值修正定时器
...
float O2C = 4;		//O2浓度 %
float NOxC;		//NOx浓度 ppm
float NOxOUT;		//NOx浓度 ppm
void Cmd_StopHandle(void);
void Can_Init(void);			//配置CAN，并初始化
void CAN_RxHandle(void);			//CAN接收中断
void AtmosphereCalculate(void);		//气氛计算
void Status_Detect(void);			//状态判断
void CAN_TxTask(void);				//CAN数据处理并发送
void CAN_TxDataTask(void);			//CAN发送数值
void CAN_CMDTransmit(uint16_t cmdid,uint8_t* data,uint32_t Timeout_ms);			//CAN发送命令
uint8_t CAN_Start = 0;			//露点启动标识
uint32_t CAN_StartTimemr = 0;	//露点启动定时器
uint32_t CAN_StopTimemr = 0;	//露点停止时间
uint8_t CAN_Stop = 0;			//露点停止标识
...
float NOxHandle(float O2C,float NOxCAN){
	...
	if(RS_flipflop((uint8_t)(NOxCAN > 50), (uint8_t)(O2C >= 20.5))){//预测模型
...
void CAN_control(void){
	can_receive();
	can_transmit();
	if(clock_time_exceed(CAN_RxManyPostbackTimemr,20)){//20ms发一次
...
void FTM1_Task(void)
{
	if(FTM_GetOverFlowFlag(FTM1)){
		...
		if(CAN_Start && CAN_StartTimemr && clock_time_exceed(CAN_StartTimemr,300*1000)){	//超时后，停止工作
			CAN_Start = 0;
			CAN_StartTimemr = 0;
			set_working_stage(STAGE_IDLE);
		}
		...
		AtmosphereCalculate();
		CAN_TxTask();//DATA1
		if(clock_time_exceed(DATA2_1STimer, 1000)){			//周期性事件 T=1s
