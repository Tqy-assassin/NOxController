...
#define PreHeat_Time	(10)	//????????  ??¦Ë??s
#define Max_preHeatTemp	(0.0800)//?????????????????????????????????????

void Temperature_Controller(void);//??????
//#define Min_Temp		(CLT_pointer()->temper - 0.01)	//??§³???????
#define Min_Temp		(CLT_pointer()->temper - 0.01)	//??§³???????
...
#define TempStabilize_Time	(5)	//?????????  ??¦Ë??s
...
//				set_pwm_dutycycle(PWM_Duty_Cycle);
				...
				if(ADtf_Value->VTempValue > 0.7*PCLT->temper){//???Ú…????70%???PID????
					perheatcount = 0;
...
//	}
	...
	Temper.E_1 = Temper.E;
	Temper.E = PCLT->temper*TCR - ADtf_Value->VTempValue;//?Ú…?-?????
